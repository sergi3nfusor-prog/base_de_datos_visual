...
with st.sidebar:
    ...
    show_stock = st.checkbox("📊 Stock por Marca", value=True)
    show_status = st.checkbox("🥧 Estado de Productos", value=True)
    show_price = st.checkbox("💰 Rangos de Precios", value=True)
    show_movement = st.checkbox("📈 Movimientos", value=True)
    show_provider = st.checkbox("🏢 Por Proveedor", value=False)
    show_timeline = st.checkbox("📅 Línea de Tiempo", value=False)
    
    st.markdown("---")
    st.info(f"✅ Gráficos activos: {sum([show_stock, show_status, show_price, show_movement, show_provider, show_timeline])}/6")
...
# Resumen por marca
st.markdown("---")
st.markdown("### 🏷️ Resumen por Marca")
